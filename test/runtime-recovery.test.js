...
test("describes server, renderer, and resume recovery without exposing URLs", () => {
  ...
  assert.equal(
    ...
    "Deskfin could not reconnect to Jellyfin after the computer resumed. Check the network.",
  );
});
...
test("maps crashed-renderer recovery choices", () => {
  const prompt = rendererRecoveryPrompt(
    "crashed",
    "Reload Deskfin or choose another server.",
  );
  assert.deepEqual(prompt.buttons, ["Reload Deskfin", "Switch server", "Quit"]);
...
test("lets an unresponsive renderer recover without forcing a reload", () => {
  ...
  assert.deepEqual(prompt.buttons, ["Wait", "Reload Deskfin", "Switch server"]);
