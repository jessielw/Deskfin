...
Jellyfin no longer has an official desktop client. Noktus is an unofficial
...
Noktus began as an exploration of several different desktop technologies before
...
Linux. That allows Noktus to behave the same on every platform, simplifies
...
memory usage, Noktus is designed to remain lightweight. Compared to some native
...
Noktus loads the official Jellyfin Web interface directly. Inline playback is
...
For videos that benefit from it, Noktus can instead open an external MPV
...
drop-in alternative to MPV. Noktus looks for regular MPV first, then mpv.net,
or you can select either executable in Settings. mpv.net keeps its own native
player window; Noktus does not modify its configuration files.
...
full native player inside Noktus. MPV is optional, and Jellyfin Web remains the
